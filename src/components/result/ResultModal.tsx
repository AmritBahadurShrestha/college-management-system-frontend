...
interface IOption { _id: string; name?: string; title?: string; fullName?: string; code?: string; [key: string]: any; }

const EXAM_TYPES = ["REGULAR", "BACK"];
const PROGRAMS = ["BCA", "BE-IT", "BE-CMP", "BE-CIVIL"];
const currentYear = new Date().getFullYear();
const EXAM_YEARS = Array.from({ length: currentYear + 1 - 2000 + 1 }, (_, i) => 2000 + i).reverse();
...
export default function ResultModal({ editTarget, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    const fetchRefs = async () => {
      try {
        setRefLoading(true);
        const [stuRes, clsRes, crsRes] = await Promise.all([
          api.get("/student"),
          api.get("/class"),
          api.get("/course"),
        ]);
  ...
  useEffect(() => {
    if (editTarget) {
      setForm({
        ...
        // Map populated marks back to IMarkInput shape
        marks:
  ...
  const handleMarkChange = (
    ...
    setForm((prev) => {
      ...
      updated[index] = { ...updated[index], [name]: parsed };
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      if (editTarget) {
        // Update only the fields the Result model owns directly;
        // cgpa/overallStatus are recomputed by the backend on mark changes
  ...
  const getCourseLabel = (c: IOption) =>
    c.code ? `${c.name ?? c.title} (${c.code})` : (c.name ?? c.title ?? c._id);
      ...
      <div className="w-full sm:max-w-2xl max-h-[95vh] sm:max-h-[90vh] flex flex-col bg-gradient-to-b from-[#0f1829] to-[#0b1120] border border-[#1e2d4a] sm:rounded-2xl rounded-t-2xl overflow-hidden animate-[slideUp_0.2s_ease] shadow-2xl shadow-black/60">
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-7 py-5 sm:py-6 space-y-6">
          ...
          {refLoading ? (
            ...
            <>
              {/* ── Result-level fields ── */}
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-widest text-[#3d4f72] mb-3">Result Details</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

                  {/* Student */}
                  <div className="flex flex-col gap-1.5">
                    <label className={labelClass}>Student</label>
                    <select name="student" value={form.student} onChange={handleChange} className={`${inputClass} cursor-pointer`}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <select name="class" value={form.class} onChange={handleChange} className={`${inputClass} cursor-pointer`}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className={labelClass}>
                      Semester <span className="normal-case font-normal tracking-normal text-[#3a4060]">(1–8)</span>
                    </label>
                    <input
                      name="semester"
                      type="number"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="flex flex-col gap-1.5 sm:col-span-2">
                          ...
                          >
                            ...
                            {courses.map((c) => (
                              <option key={c._id} value={c._id}>{getCourseLabel(c)}</option>
                            ))}
                          </select>
                        </div>
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className={labelClass}>
                            Mark <span className="normal-case font-normal tracking-normal text-[#3a4060]">(0–100)</span>
                          </label>
                          <input
                            name="mark"
                            type="number"
                        ...
                        <div className="flex items-center gap-2.5 self-end pb-2.5">
                          ...
                          />
                          <label htmlFor={`absent-${i}`} className={`${labelClass} cursor-pointer normal-case tracking-normal font-medium text-[#8896b3]`}>
                            Absent
                          </label>
                        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-5 sm:px-7 py-4 sm:py-5 border-t border-[#1a2540] bg-[#080e1c]/50">
          ...
          >
            ...
            ) : editTarget ? "Save Changes" : "Create Result"}
          </button>
        </div>
      </div>
